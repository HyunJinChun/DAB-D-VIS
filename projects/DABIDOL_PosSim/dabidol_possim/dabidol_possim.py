...
from .models.deformable_detr import DABDeformableDETR, SetCriterion
...
class DABIDOL_PosSim(nn.Module):
    """
    Implement DAB-IDOL_PosSim
    """
    ...
    def __init__(self, cfg):
        ...
        # print('MEMORY_LEN(T) :', self.memory_len)
        ...
        self.match_score_thr = cfg.MODEL.IDOL.MATCH_SCORE_THR
        print('MATCH SCORE THR :', self.match_score_thr)
        # import sys; sys.exit()

        # * cascade in inference setting
        self.cascade = cfg.MODEL.IDOL.CASCADE
        # print('CASCADE :', self.cascade)

        # * position similarity in inference setting
        self.possim = cfg.MODEL.IDOL.POSSIM
        ...
        # * DAB-Deformable DETR
        two_stage = cfg.MODEL.IDOL.TWO_STAGE
        use_dab = cfg.MODEL.IDOL.USE_DAB
        num_patterns = cfg.MODEL.IDOL.NUM_PATTERNS
        random_refpoints_xy = cfg.MODEL.IDOL.RANDOM_REFPOINTS_XY

        ## ******* 이 부분은 TDAB_hyun/tdab_base.py의 def build(args)와 동일함. 참고할 것 ******* ##
        ...
        self.detr = CondInst_segm(model, freeze_detr=False, rel_coord=True, use_dab=use_dab)
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            if video_len > clip_length:
                ...
                if self.cascade:  # * cascading in inference
                    cascade = None
                    for c in range(num_clips):
                        ...
                        clip_output, cascade = self.detr.inference_forward_cascade(clip_images, size_divisib=32,
                                                                                   cascade=cascade)
                ...
                images = self.preprocess_image(batched_inputs)
                if self.cascade:  # * cascading in inference
                    output, cascade = self.detr.inference_forward_cascade(images, size_divisib=32, cascade=None)
                else:  # no cascading
        ...
        for inst_id, m in enumerate(video_dict.keys()):
            ...
            for n in range(vid_len):
                ...
                if mask_i is None:
                    ...
                    pred_mask_i = (F.interpolate(pred_mask_i, size=(ori_size[0], ori_size[1]), mode='nearest') > 0.5)[
