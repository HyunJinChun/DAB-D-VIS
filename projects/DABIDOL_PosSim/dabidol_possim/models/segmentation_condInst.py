...
class CondInst_segm(nn.Module):
    def __init__(self, detr, rel_coord=True, freeze_detr=False, use_dab=True):
        super().__init__()
        self.detr = detr
    ...
    def forward(self, samples, det_targets, ref_targets, criterion, train=False):
        ...
        if not self.use_dab:
            ...
            tgt_embed = self.detr.tgt_embed.weight  # nq, 256
            refanchor = self.detr.refpoint_embed.weight  # nq, 4
        ...
        # HYUN #
        # 이 부분이 IDOL 구조도 상에서 공통적인 Object Queries를 받아서 Transformer(Deformable DETR)를 수행하는 부분
        # hs : Key Frame의 임베딩 된 최종 Object Query
        hs, memory, init_reference, inter_references, _un1, _un2, _un3 = self.detr.transformer(srcs_key, masks_key, poses_key, query_embeds) # Key Frame
        hs_ref, memory_ref, init_reference_ref, inter_references_ref, _un1, _un2, _un3 = self.detr.transformer(srcs_reference, masks_reference, poses_reference, query_embeds) # Reference Frame
    ...
    def inference_forward_cascade(self, samples, size_divisib=32, cascade=None):
        image_sizes = samples.image_sizes
        if not isinstance(samples, NestedTensor):
            samples = nested_tensor_from_tensor_list(samples, size_divisibility=size_divisib)
        ...
        hs, memory, init_reference, inter_references, inter_samples, enc_outputs_class, enc_outputs_coord_unact = \
            self.detr.transformer.forward_cascade(srcs, masks, poses, query_embeds, cascade)
        ...
        return outputs, inter_references[-1]
