...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None):
        ...
        if self.two_stage:
            ...
            # HYUN #
            # 여기서 앞선 프레임의 query_embed를 가져오면 될 듯 => self.use_dab를 사용하기 때문에 아래의 내용 수정 필요
            # (1) Anchor/Bounding Box들만 넘겨받는 구조
            # 앞선 프레임의 inter_references를 추가적인 매개변수로 받아서 진행, query_embed는 코드 그대로 사용
            # (2) Anchor/Bounding Box + Instance 별 Decoded Feature Vector들도 넘겨받는 구조
            # 앞선 프레임의 query_embed 자체를 모두 받아서 진행, 코드 디버깅을 통해 query_embed 차원 확인 필요
