...
class CondInst_segm(nn.Module):
    ...
    def forward(self, samples, det_targets, ref_targets, criterion, train=False):
        ...
        hs, memory, init_reference, inter_references, _un1, _un2, _un3 = \
            self.detr.transformer(srcs_key, masks_key, poses_key, query_embeds) # Key Frame
        hs_ref, memory_ref, init_reference_ref, inter_references_ref, _un1, _un2, _un3 = \
            self.detr.transformer(srcs_reference, masks_reference, poses_reference, query_embeds) # Reference Frame
        ...
        for lvl in range(enc_lay_num):
            ...
            # for training & log evaluation loss
                                 # criterion.matcher : matcher.py/HungarianMatcher()
        ...
        reference_ref = inter_references_ref[-1]
        tmp = self.detr.bbox_embed[-1](hs_ref[-1])
        if reference_ref.shape[-1] == 4:
            new_reference_points = tmp + inverse_sigmoid(reference_ref)
            new_reference_points = new_reference_points.sigmoid()
        else:
            assert reference_ref.shape[-1] == 2
            new_reference_points = tmp
            new_reference_points[..., :2] = tmp[..., :2] + inverse_sigmoid(reference_ref)
            new_reference_points = new_reference_points.sigmoid()
        ref_box = new_reference_points

        contrast_items = select_pos_neg(ref_box, matched_ids, ref_targets, det_targets,
        ...
        if train:
                      # criterion: deformable_detr.py/SetCriterion()
